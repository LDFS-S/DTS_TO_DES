...
class SpeedParameters:
    def __init__(self):
        ...
        self.parameters = {
            ...
            21: (5.32116684554369, 0.7661712009389063),
            ...
            51: (1.6805616568000001, 0.18983218185506565),
            52: (1.6805616568000001, 0.18983218185506565),
            53: (1.5839512520526313, 0.170782626145043),
            54: (1.5174037679145302, 0.32361150412208217),
            55: (1.4653246006212486, 0.3259503587810072)
        }

    def get(self, order_density):
        # 返回给定密度等级的速度参数（均值和标准差）
        return self.parameters.get(order_density)
...
def calculate_max_speed_based_on_order_density(max_order_density, car, current_time):
    ...
    max_next_station = stations[(car.current_station.index + 1) % num_stations]
    max_current_station = car.current_station
    previous_car = None
    ...
    for event in global_event_queue:
        if event.car == previous_car and event.event_type == "Arrival":
            earliest_time_previous_car = event.time
            break
    else:
        print("未找到前车到达事件")
        # 如果未找到前车到达事件，返回稍微低于理论最大速度的值
        return None
    # print(car.number, previous_car.number)
    # 计算当前车辆到达相同位置的最短时间
    ...
    # 计算最大速度，确保在此时间内不追上前车
    max_speed_space_based = (
                                        distance_to_next_car - safe_distance + previous_car.velocity * time_to_reach_same_position) / time_to_reach_same_position
    # print("max", min(max_speed_space_based, mean_speed + 3 * std_dev), car.number)
    # 确保计算出的速度在合理范围内
    return min(max_speed_space_based, mean_speed + 3 * std_dev)


def calculate_min_speed_based_on_order_density(order_density, car, current_time):
    mean_speed, std_dev = speed_parameters.get(order_density)
    ...
    for potential_next_car in cars:
        if potential_next_car.current_station == car.current_station and potential_next_car.order == car.order + 1:
            ...
            slowest_speed_next_car = next_mean_speed - 3 * next_std_dev if next_mean_speed is not None else mean_speed - 3 * std_dev
            break
    else:
    ...
    # 计算后车的最早可能到达时间
    time_next_car_earliest = current_time + calculate_circular_distance(next_station.position, next_car.position,
                                                                        road_length) / slowest_speed_next_car
    ...
    # 计算最小可能速度
    distance_to_next_station = calculate_circular_distance(next_station.position, car.position, road_length)
    min_speed = distance_to_next_station / time_to_reach_same_position

    # 确保计算出的速度在合理范围内
    # print("min", max(min_speed, mean_speed - 3 * std_dev), car.number, "\n")
    return max(min_speed, mean_speed - 3 * std_dev)
...
# 添加初始到站事件
# 初始化事件
sorted_cars = sorted(cars, key=lambda x: x.number, reverse=True)
...
for car in sorted_cars:
    ...
    while initial_arrival_time is None:
        ...
        earlier_arrivals = [event.time for event in global_event_queue if event.car.current_station == current_station]
...
def recalculate_arrival_time_for_station(recalculate_arrival_station, current_time):
    ...
    for car in station_cars:
        ...
        recalculate_arrival_order_density = car.order * 10 + (
            stations[(car.current_station.index + 1) % num_stations]).car_count
        if car.order > 0:
            previous_car = station_cars[station_cars.index(car) - 1]
            recalculate_arrival_order_density_max_speed = calculate_max_speed_based_on_order_density(
                recalculate_arrival_order_density, car, current_time)

        if car.order < len(station_cars) - 1:
            recalculate_arrival_order_density_min_speed = calculate_min_speed_based_on_order_density(
                recalculate_arrival_order_density, car, current_time)
...
def check_for_overtaking(car, next_arrival_time):
    # 获取当前车辆的前车
    ...
    if previous_car:
        ...
        previous_car_arrival_time = next(
            (event.time for event in global_event_queue if event.car == previous_car and event.event_type == "Arrival"),
            None)

        if previous_car_arrival_time and next_arrival_time < previous_car_arrival_time:
...
# 更新函数
def update(frame):
    global update_count, info_printed, cars, interval_duration, warm_up_phase, global_event_queue, iteration_count, stations
    ...
    if next_event is not None:
        ...
        if next_event.event_type == "Arrival":
            ...
            if next_station.car_count < max_cars_per_station:
                ...
                print(
                    f"Car {car.number} arrived at Station {next_station.index + 1} at simulation time {current_time:.2f}")
                # 车辆离开站点时，当前站点车辆前往下个站点的事件时间不会重新计算，导致后面再进站的车辆计算的下个到达事件时间是根据当前密度来判断的，所以可能使用比前车更短的时间

                car_information = (car.current_station.index + 1, current_time, car.velocity, car.position)
                ...
                car.order = next_station.car_count - 1
                ...
                order_density = car.order * 10 + after_next_station.car_count
                ...
                car_max = calculate_max_speed_based_on_order_density(order_density, car, current_time)
                car.velocity = generate_speed(order_density, final_max_speed=car_max)

                travel_time = station_length / car.velocity
                next_arrival_time = current_time + travel_time
                check_for_overtaking(car, next_arrival_time)
                ...
                recalculate_arrival_time_for_station(current_station, current_time)
                waiting_station_index = current_station.index
                ...
                if current_station.waiting_queue:
                    while True:
                        ...
                        print(
                            f"Car {car_to_arrive.number} arrived at Station {waiting_station.index + 1} at simulation time {current_time:.2f}")
                        ...
                        car_information = (
                        car_to_arrive.current_station.index + 1, current_time, car_to_arrive.velocity,
                        car_to_arrive.position)
                        ...
                        MAX_CAR_TO = calculate_max_speed_based_on_order_density(car_to_arrive_order_density,
                                                                                car_to_arrive,
                                                                                current_time)
                        ...
                        if waiting_station_index == current_station.index or not stations[
                            waiting_station_index].waiting_queue:
                            break
            else:
                # 下一个站点已满，将车辆放入下一个站点的等待队列
                next_station.waiting_queue.append(car)
                print(
                    f"Car {car.number} arrived at waiting queue {next_station.index + 1} at simulation time {current_time:.2f}")
    return []
